...
#define MAX_RECOMMENDED_LITERS 5
// #define THREE_FOURTHS_RECOMMENDED_LITERS (MAX_RECOMMENDED_LITERS * 0.75)
#define PULSE_TO_LITER(pulseCount) (pulseCount / (float)PULSE_LITER_NUM)
#define MAX_LENGTH_SHOWER_TIME 7200 /* Max shower in seconds */
...
#define MAX_TIME_NO_WATER_FLOW_ALLOWED_MS (1 * 1000)
...
static TaskHandle_t buzzerHandle = NULL;
...
SemaphoreHandle_t xSemaphoreWaterFlowing3;
...
void IRAM_ATTR waterFlowISR(void* arg) {
  ...
  xSemaphoreGive(xSemaphoreWaterFlowing3);
}
...
void printTemperatureGraphWithAverage() {
  ...
  for (int i = 0; i < systemData.temp_graph_index; i++) {
    ...
    printf("Index %d - Time: %u, Temp: %u, Water Rate: %.2f\n", i, entry.time,
           entry.temp, entry.currWaterRate);
  }
...
void handlePostShowerState() {
  ...
  systemData.currShowerSessionTime -=
      (MAX_TIME_NO_WATER_FLOW_ALLOWED_MS / 1000);
...
void handleSetupState() {
  ...
  char password[32];
    ...
    bool hi = save_wifi_credentials_to_nvs("Shib", "12345678");
...
void handleShowerState() {
  ...
  if (xSemaphoreTake(xSemaphoreWaterFlowing1,     
                     pdMS_TO_TICKS(MAX_TIME_NO_WATER_FLOW_ALLOWED_MS)) ==
      pdTRUE) {
    ...
    vTaskSuspend(buzzerHandle);
    printf(
        "No Water Flow, Switching to POST_SHOWER_PROCESSING_STATE at Line: "
        "%d\n",
        __LINE__);
  }
}
...
void handleTransmitState() {
  ...
  if (success) {
    ...
    char buf[100] = {'\0'};
    sprintf(buf, "Total Liters Used %f\nAverage Temp: %f, Total Pulses: %ld", systemData.litersUsed, systemData.averageWaterTemp, systemData.totalPulses);
    //mqtt_app_start_wrapper(buf, sizeof(buf));
    //place MQTT functions here
    mqtt_app_start(buf);
  } else {
    printf("Failed to connect\n");
  }

  vTaskDelay(50000 / pdMS_TO_TICKS(1000));
...
void loggerThread(void* parameter) {
  ...
  for (;;) {
    ...
    systemData.currShowerSessionTime =
        pdTICKS_TO_MS(currTick - systemData.showerSessionStartTime) / 1000;
    ...
    if ((systemData.totalPulses - lastPulseCount) <= PULSE_HYSTERISIS) {
      ...
      // printf("[Line %d] - No water flow detected, created logEntry with zero
      // flow.\n", __LINE__);
      systemData.temp_graph[systemData.temp_graph_index] = logEntry;
      // printf("[Line %d] - Updated temp_graph at index %u.\n", __LINE__,
      // systemData.temp_graph_index);
    } else {
      systemData.currWaterRate =
          (float)(PULSE_TO_LITER(systemData.totalPulses) -
                  PULSE_TO_LITER(lastPulseCount)) /
          (pdTICKS_TO_MS(elapsedTicks) / 1000.0 / 60.0);

      dataPoint logEntry = {systemData.currShowerSessionTime,
                            (uint8_t)systemData.currWaterTemp,
                            (float)(PULSE_TO_LITER(systemData.totalPulses) -
                                    PULSE_TO_LITER(lastPulseCount))};
      // printf("[Line %d] - Water flow detected, created logEntry with flow
      // rate.\n", __LINE__);
      systemData.temp_graph[systemData.temp_graph_index] = logEntry;
      // printf("[Line %d] - Updated temp_graph at index %u.\n", __LINE__,
      // systemData.temp_graph_index);
    ...
    systemData.temp_graph_index =
        (systemData.temp_graph_index + 1) % temp_graph_length;
    lastPulseCount = systemData.totalPulses;


    /* If recommned water use excedded, resume the buzzerTask*/ 
    if ( PULSE_TO_LITER(systemData.totalPulses) > MAX_RECOMMENDED_LITERS ) {
      vTaskResume(buzzerHandle);
    }
...
void buzzerThread(void* parameter) {
  uint32_t delay = 5000; // start delay of 10,000
  for(;;) {
    if(xSemaphoreTake(xSemaphoreWaterFlowing3, 0) == pdTRUE) {
      //Turn buzzer on for half a second with 4.2 second interval (starting)
      gpio_set_level(BUZZER_PIN, 1);
      vTaskDelay(BUZZER_ON_TIME / portTICK_PERIOD_MS);
      gpio_set_level(BUZZER_PIN, 0);
      vTaskDelay(BUZZER_DELAY_TIME(systemData.totalPulses) / portTICK_PERIOD_MS);
    }
  }
}
...
void app_main() {
  ...
  xSemaphoreWaterFlowing3 = xSemaphoreCreateBinary();
  printf("Line %d: Created binary semaphore 3\n", __LINE__);
  ...
  BaseType_t res3 = xTaskCreate(loggerThread, LOGGER, LOGGER_STACK_SIZE, NULL,
                                LOGGER_PRIORITY, &loggerHandle);

  BaseType_t res4 = xTaskCreate(buzzerThread, BUZZER, BUZZER_STACK_SIZE, NULL, BUZZER_PRIORITY, &buzzerHandle);
